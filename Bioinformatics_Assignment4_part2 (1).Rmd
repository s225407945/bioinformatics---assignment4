---
title: "Bioinformatics Assignment 4 — Part 2"
...
knitr::kable(head(codon_tab[order(-abs(codon_tab$Diff)), ], 10),
             caption = "Top 10 codons by |$\\Delta$RSCU| (preview)")
...
barplot(t(as.matrix(top[, c("RSCU_Ecoli","RSCU_Cryo")])),
        ...
        main = "Top 20 codons by |$\\Delta$RSCU|")
...
**Explanation:**  
RSCU normalises codon counts within each synonymous family to highlight **preference** independent of amino-acid usage. Large |$\\Delta$RSCU| values indicate codons preferred in one organism over the other. Differences typically track **GC bias** (e.g., GC-ending codons enriched in GC-rich genomes), translational selection, and tRNA pools.
...
For each k \\in {3,4,5}, k-mer frequencies were computed over the 20-AA alphabet and normalised to frequencies (`freq = TRUE`).  
We aligned the k-mer sets, computed $\\Delta$freq = (Cryo $-$ E. coli), exported full tables, and plotted the **top 10 over-represented** and **top 10 under-represented** k-mers in *Cryobacterium*.
...
compare_kmers <- function(k) {
  ...
  barplot(top10$Diff, names.arg = top10$Kmer, las = 2,
          ...
          ylab = "Frequency difference ($\\Delta$freq)")
  grid(); dev.off()
...
**Display figures:**
```{r p2-q6-show, echo=FALSE, fig.align='center'}
...
Comparing the Cryo $\\Delta$freq with *E. coli* shows whether the same motifs are preferred to a **similar extent** ($\\Delta$freq $\\approx 0$) or **differ markedly** (|$\\Delta$freq| large), consistent with distinct evolutionary pressures and habitat adaptation.
